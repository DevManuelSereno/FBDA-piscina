...
import type {
  MelhoresDoAnoColetivoItem,
  MelhoresDoAnoIndividualItem,
} from "@/lib/melhores-do-ano";
...
export type FiltrosLabelMelhoresDoAno = {
  circuitoNome?: string;
  temporada?: string;
};
...
function Cabecalho({
  ...
  titulo: string;
  filtros: FiltrosLabelMelhoresDoAno;
}) {
  const partesFiltro = [
    filtros.temporada ? `Temporada ${filtros.temporada}` : "Todas as temporadas",
...
export function MelhoresDoAnoIndividualPdf({
  itens,
  filtros,
}: {
  itens: MelhoresDoAnoIndividualItem[];
  filtros: FiltrosLabelMelhoresDoAno;
}) {
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <Cabecalho titulo="Melhores do Ano — Individual" filtros={filtros} />
...
export function MelhoresDoAnoColetivoPdf({
  itens,
  filtros,
}: {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <Cabecalho
          titulo="Melhores do Ano — Coletivo (por Clube)"
          filtros={filtros}
        />
        <View style={[styles.tabela, { marginHorizontal: 32 }]}>
          <View style={styles.linhaHeader}>
...
export function criarDocumentoMelhoresDoAno(
  ...
  if (modo === "coletivo") {
    return <MelhoresDoAnoColetivoPdf itens={dados.coletivo} filtros={filtros} />;
  }
  return <MelhoresDoAnoIndividualPdf itens={dados.individual} filtros={filtros} />;
}
