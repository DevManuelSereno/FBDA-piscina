...
- **Segundo bug de framework no mesmo `Popover`, descoberto após o usuário reportar que os seletores de mês/ano do `Calendar` (`captionLayout="dropdown"`) fechavam o Popover sem aplicar a escolha:** o `<select>` nativo desses seletores abre seu dropdown fora da árvore do DOM (renderizado pelo navegador/SO), o que o Base UI interpreta como o foco saindo do Popover — disparando `onOpenChange(false, { reason: "focus-out" })` e fechando o Popover (e desmontando o `Calendar`, por causa do fix anterior de mount condicional) antes do `<select>` registrar a escolha. Corrigido inspecionando o `reason` do evento e ignorando o fechamento quando `reason === "focus-out"`, mantendo os demais motivos (clique fora, Esc, seleção de dia) funcionando normalmente.
