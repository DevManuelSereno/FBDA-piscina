...
## Polimento de UI/UX pós-Fase 2 (auditoria `impeccable` — ver `docs/adjustments.md`)

Plano completo (Fases A–F + G1) executado em sequência: `EmptyState` compartilhado, ordenação de colunas na `DataTable`, `grupoRelatorio` validado por select, dark mode via `next-themes`, toasts (`sonner`) + skeletons (`loading.tsx`) de carregamento, correções de acessibilidade/i18n, sombra de scroll nas tabelas, limpeza de dependências mortas, e um date picker (`Calendar` + `Popover`) no formulário de Atletas.

**Correções em relação ao plano original, descobertas só na execução:**
- `shadcn` (o pacote npm) **não é só a CLI de scaffolding** — `app/globals.css` importa `shadcn/tailwind.css` (`@import "shadcn/tailwind.css";`), consumido pelo Tailwind em build/runtime. Mover para `devDependencies` (como o plano original sugeria) quebra o build (`CssSyntaxError: Can't resolve 'shadcn/tailwind.css'`). Mantido em `dependencies`.
- **Bug de framework confirmado no `Popover` do Base UI (mesma família de `Menu`/`Dialog` — ver acima):** ao selecionar uma data no `Calendar` dentro do `Popover` do date picker de Atletas, `setOpen(false)` nunca completa a transição de fechamento — o `[data-slot=popover-content]` permanece no DOM e visível indefinidamente (mesmo sintoma do bug já documentado em `Dialog`/`AlertDialog`). Corrigido com o mesmo padrão: montar o conteúdo condicionalmente (`{calendarioAberto && <PopoverContent>...}`) em vez de deixar o Base UI gerenciar o unmount via seu próprio ciclo de animação.
- **Novo componente `components/ui/calendar.tsx`, `popover.tsx`, `field.tsx`, `input-group.tsx`** instalados via `npx shadcn@latest add` (trouxe `react-day-picker` como dependência nova). Integração do date picker com o formulário nativo (`<form action={serverAction}>` + `FormData`, sem React Hook Form): um `<input type="hidden" name="dataNascimento">` em formato ISO (`yyyy-mm-dd`, o que `z.coerce.date()` espera) é mantido em sincronia com o `Calendar` (controlado) e o campo de texto visível (formato `pt-BR`, editável manualmente com parse e validação de data real via `parseDataPtBr`).
- **Segundo bug de framework no mesmo `Popover`, descoberto após o usuário reportar que os seletores de mês/ano do `Calendar` (`captionLayout="dropdown"`) fechavam o Popover sem aplicar a escolha:** o `<select>` nativo desses seletores abre seu dropdown fora da árvore do DOM (renderizado pelo navegador/SO), o que o Base UI interpreta como o foco saindo do Popover — disparando `onOpenChange(false, { reason: "focus-out" })` (ou, dependendo do navegador, `"outside-press"` no clique que confirma a opção) e fechando o Popover (e desmontando o `Calendar`, por causa do fix anterior de mount condicional) antes do `<select>` registrar a escolha.
  - Primeira tentativa (insuficiente): ignorar só `reason === "focus-out"` — o usuário reportou que o bug persistia, porque o motivo real varia (o clique que confirma a opção no dropdown nativo também pode ser lido como `"outside-press"`).
  - Segunda tentativa (também insuficiente): trocar a checagem por texto do `reason` por uma verificação de alvo do evento (`eventDetails.event.target`/`trigger`/`document.activeElement` dentro de `[data-slot="calendar"]`) — funcionou nos testes automatizados (eventos sintéticos via `dispatchEvent`), mas o usuário confirmou por vídeo que o bug persistia com cliques reais: o próprio clique para **abrir** o `<select>` de mês/ano já fechava o Popover instantaneamente, antes do dropdown nativo sequer aparecer. Isso revelou que testar com eventos sintéticos não é equivalente a um clique real do navegador aqui — o caminho interno do Base UI que decide "isso é um clique fora" claramente se comporta diferente para um `PointerEvent` disparado pelo SO vs. um despachado via JS, então validar só com `dispatchEvent` deu falso positivo.
  - Terceira tentativa (também insuficiente — causa raiz errada): trocar `Popover` por `Dialog` (usando o `DialogContent` padrão do projeto) para hospedar o `Calendar`, na hipótese de que o problema era a heurística de "clique fora" do `Popover`. O usuário reportou por print que o bug persistia **idêntico** mesmo com `Dialog` — a mesma ação (clicar para abrir o `<select>` de mês/ano) continuava fechando o diálogo na hora. Isso provou que a causa não era específica do `Popover`.
  - **Causa raiz real, só identificada depois de eliminar as hipóteses acima:** `<select>` nativo **não abre de forma confiável quando um ancestral tem `transform` CSS aplicado** — bug cross-browser conhecido (o navegador posiciona/renderiza o dropdown nativo em relação ao espaço de coordenadas transformado do ancestral, e o popup pode falhar ao abrir ou fechar instantaneamente). O `DialogContent` padrão do projeto centraliza via `-translate-x-1/2 -translate-y-1/2` — exatamente um `transform`. Como `Popover` e `Dialog` usam a mesma estratégia de centralização por transform, trocar um pelo outro não mudava nada; o `<select>` continuava dentro de um ancestral transformado em ambos os casos.
  - **Fix definitivo:** para este date picker especificamente, o popup do calendário é montado manualmente com os primitivos crus do Base UI (`DialogPortal` + `DialogOverlay` + `DialogPrimitive.Popup`), centralizado por **flexbox** (`fixed inset-0 flex items-center justify-center`) em vez de `transform` — confirmado via `getComputedStyle(...).transform === "none"` em toda a cadeia de ancestrais do `<select>` até o `<body>`. O `DialogContent` compartilhado (usado nos outros 13+ diálogos, todos com `NativeSelect` comuns, não com `<select>` de calendário) não foi alterado — o problema só se manifestava aqui pela combinação específica de um `<select>` de terceiros (`react-day-picker`) dentro de um segundo nível de diálogo aninhado.
  - Ajuste de layout junto: o cabeçalho do popup do calendário (título + botão fechar) usa `flex items-center justify-between` em vez de posicionamento absoluto do botão — com o `DialogTitle` absoluto anterior, o texto "Selecionar data de nascimento" colidia com o X (a caixa é estreita, do tamanho do `Calendar`, sem largura reservada pro botão). Título também encurtado para "Data de nascimento".
